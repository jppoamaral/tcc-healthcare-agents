...
Simulates the conversation flow:
  ...
  Turn 3: "preciso cancelar minha consulta"
          → cancel_appointment → confirms cancellation
...
def main() -> None:
    print("=" * 65)
    print("  TEST: 3-turn cancel scenario")
    print("=" * 65)
    print()
    ...
    try:
        ...
        # ============================================================== #
        # TURN 3 — cancel the appointment
        # ============================================================== #
        print("-" * 65)
        print("TURN 3: \"preciso cancelar minha consulta\"")
        print("-" * 65)
        ...
        print(f"    appointment: 2025-07-21 09:00, Dr. Ricardo Lopes")
        ...
        ok3 = (
            "result" in r3
            and r3["result"].get("status") == "cancelled"
            and r3["result"]["cancelled_appointment"]["doctor"] == "Dr. Ricardo Lopes"
            and r3["result"]["cancelled_appointment"]["date"] == "2025-07-21"
            and r3["result"]["cancelled_appointment"]["time"] == "09:00"
            and r3["result"]["cancelled_appointment"]["patient_name"] == PATIENT_INFO["name"]
            and r3["result"]["cancelled_appointment"]["cpf"] == PATIENT_INFO["cpf"]
        )
        status3 = "PASS" if ok3 else "FAIL"
        print(f"  Clinic A responded: status={r3.get('result', {}).get('status')}")
        print(f"  Cancelled: {r3.get('result', {}).get('cancelled_appointment')}")
        print(f"  Verifier → SAFE (mocked)")
        print(f"  => {status3}")
        if ok3:
            passed += 1
