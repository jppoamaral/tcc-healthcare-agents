...
| 4.0.0 | 2025-08 | Infraestrutura de testes automatizada: executor de lote (`executar_testes.py`), suite de 30 casos CSV em 9 categorias (`casos_teste.csv`), avaliador de metricas (`avaliar_metricas.py`). Metricas atualizadas: HMR substitui HR/PVR/MCRA. Scripts de inicializacao: `start_clinics.sh` (A+B) e `start_all_clinics.sh` (todas). Log estruturado JSON Lines (`logs.jsonl`). Resultados baseline: TSR=86.7%, TCA=100.0%, HMR=0.0%. |
...
As seguintes metricas fornecem evidencia quantitativa para avaliacao da tese. Todas as medicoes sao derivadas de logs estruturados capturados pelo executor de testes automatizado (`tests/executar_testes.py`) e calculadas pelo avaliador de metricas (`tests/avaliar_metricas.py`).

### 5.1 Taxa de Sucesso de Tarefa (TSR)

Mede a proporcao de casos de teste que completam o pipeline inteiro (Planner -> Router -> Verifier -> Response Generator) com uma resposta final valida.

$$
TSR = \frac{\text{Casos com } final\_response\_ok = true}{\text{Total de casos executados}} \times 100
$$

**Metodo de medicao:** O executor (`executar_testes.py`) registra `final_response_ok` para cada caso no log JSONL. O avaliador conta os casos com `final_response_ok == true` e divide pelo total.
...
**Resultado baseline:** **86.7%** (26/30 casos bem-sucedidos).

### 5.2 Precisao de Chamada de Ferramenta (TCA)

Mede a capacidade do Planner de gerar os passos corretos (clinica + acao) em relacao ao esperado pela suite de testes CSV.

$$
TCA = \frac{\text{Passos onde } clinic \text{ E } action \text{ coincidem com o esperado no CSV}}{\text{Total de passos esperados}} \times 100
$$

**Metodo de medicao:** O avaliador (`avaliar_metricas.py`) compara cada passo executado (campo `steps` do log) contra as colunas `clinicas_esperadas` e `acoes_esperadas` do CSV. Um passo e correto quando tanto a clinica quanto a acao coincidem com a especificacao.

**Resultado baseline:** **100.0%** (todos os passos gerados coincidem com o esperado).

### 5.3 Taxa de Mitigacao de Alucinacoes (HMR)

Mede a eficacia do Verifier em bloquear respostas inseguras que contem alucinacoes ou conteudo potencialmente perigoso detectado durante o pipeline.

$$
HMR = \frac{\text{Casos inseguros bloqueados pelo Verifier}}{\text{Total de casos com alucinacao detectada}} \times 100
$$

**Metodo de medicao:** O executor marca `had_raw_hallucination = true` quando detecta conteudo potencialmente inseguro (ex.: dados de privacidade expostos, conteudo fora de escopo). O avaliador filtra os logs com `had_raw_hallucination == true` e verifica quantos desses foram bloqueados pelo Verifier (`verifier_safe == false`). Se nenhum caso inseguro e detectado, HMR e definida como 0.0% (sem oportunidade de mitigacao).

**Resultado baseline:** **0.0%** (nenhum caso com alucinacao detectada no baseline -- o pipeline nao gerou conteudo inseguro).

### 5.4 Protocolo de Avaliacao

#### 5.4.1 Suite de Casos de Teste (`tests/casos_teste.csv`)

A suite contem **30 casos de teste** distribuidos em **9 categorias** de intencao, cobrindo todas as especialidades e cenarios operacionais do sistema:

**Schema do CSV:**

| Coluna | Tipo | Descricao |
|---|---|---|
| `id_caso` | `int` | Identificador unico do caso (1-30). |
| `texto_usuario` | `str` | Consulta em linguagem natural (portugues). |
| `intencao_esperada` | `str` | Categoria de intencao esperada. |
| `especialidade` | `str` | Especialidade medica alvo (`cardiologia`, `dermatologia`, `ortopedia`, `invalida`). |
| `clinicas_esperadas` | `str` | Clinicas que devem ser acionadas (separadas por `;`). |
| `acoes_esperadas` | `str` | Acoes/ferramentas esperadas (separadas por `;`). |

**Distribuicao por categoria de intencao:**

| Categoria | Descricao | Casos |
|---|---|---|
| `listar` | Listar horarios disponiveis | Consultas por especialidade |
| `agendar` | Agendar consulta | Agendamento com medico/data/hora |
| `remarcar` | Reagendar consulta existente | Mudanca de data/hora |
| `cancelar` | Cancelar consulta existente | Cancelamento com dados do historico |
| `erro_privacidade` | Tentativa de acesso a dados sensiveis | Requisicoes de listagem de pacientes |
| `fora_de_escopo` | Consulta nao relacionada a saude | Perguntas genericas, saudacoes |
| `especialidade_invalida` | Especialidade nao cadastrada | Ex.: "neurologista" |
| `informacao_insuficiente` | Dados incompletos para acao | Ex.: agendar sem especificar medico |
| `buscar_paciente` | Busca por paciente especifico | Consulta por ID de paciente |

#### 5.4.2 Executor de Testes (`tests/executar_testes.py`)

O executor automatiza a submissao dos 30 casos pelo pipeline real do sistema:

1. Carrega os casos de `casos_teste.csv`.
2. Para cada caso:
   a. Envia `texto_usuario` ao **Planner** para decomposicao.
   b. Detecta saudacoes e consultas fora de escopo.
   c. Despacha os passos gerados via **Router** para as clinicas.
   d. Detecta conteudo potencialmente inseguro (palavras-chave de privacidade).
   e. Submete resultados agregados ao **Verifier** para validacao.
   f. Se `verifier.safe == true`, gera resposta final via **Response Generator**.
3. Registra o resultado no arquivo de log JSONL.

#### 5.4.3 Log Estruturado (`tests/logs.jsonl`)

Cada linha do arquivo de log e um objeto JSON independente com o seguinte schema:

| Campo | Tipo | Descricao |
|---|---|---|
| `id_caso` | `int` | Identificador do caso de teste. |
| `user_text` | `str` | Texto normalizado da consulta do usuario. |
| `steps` | `list[dict]` | Passos executados, cada um com `clinic` e `action`. |
| `verifier_safe` | `bool` | Resultado da validacao do Verifier. |
| `verifier_reason` | `str` | Justificativa do Verifier (vazio se seguro). |
| `final_response_ok` | `bool` | Se a resposta final foi gerada com sucesso. |
| `had_raw_hallucination` | `bool` | Se conteudo potencialmente inseguro foi detectado. |
...
#### 5.4.4 Avaliador de Metricas (`tests/avaliar_metricas.py`)

O avaliador carrega o CSV (especificacao esperada) e o JSONL (resultados reais) e calcula as tres metricas:

1. **TSR** -- conta `final_response_ok == true` / total.
2. **TCA** -- compara `steps` do log contra `clinicas_esperadas` + `acoes_esperadas` do CSV.
3. **HMR** -- filtra `had_raw_hallucination == true`, verifica `verifier_safe == false`.

#### 5.4.5 Resultados Baseline

| Metrica | Resultado | Interpretacao |
|---|---|---|
| **TSR** | 86.7% (26/30) | Aceitavel. 4 casos falharam na geracao de resposta. |
| **TCA** | 100.0% | Excelente. Todos os passos gerados pelo Planner estao corretos. |
| **HMR** | 0.0% | Sem oportunidade de mitigacao -- nenhuma alucinacao detectada no baseline. |
...
### Fase 5 -- Infraestrutura de Testes e Avaliacao Quantitativa

| Tarefa | Detalhe | Artefato |
|---|---|---|
| 5.1 | Projetar suite de 30 casos de teste em 9 categorias (CSV) | `tests/casos_teste.csv` |
| 5.2 | Implementar executor de testes em lote pelo pipeline real | `tests/executar_testes.py` |
| 5.3 | Implementar avaliador de metricas (TSR, TCA, HMR) | `tests/avaliar_metricas.py` |
| 5.4 | Criar script de inicializacao rapida (Clinicas A+B) | `clinic_agents/start_clinics.sh` |
| 5.5 | Criar script de inicializacao completa (todas as 6 clinicas) | `clinic_agents/start_all_clinics.sh` |
| 5.6 | Executar suite completa e gerar log estruturado JSON Lines | `tests/logs.jsonl` |
| 5.7 | Calcular metricas baseline a partir dos logs (TSR=86.7%, TCA=100.0%, HMR=0.0%) | Tabela resumo de metricas |

**Entregavel:** Infraestrutura de avaliacao quantitativa automatizada com resultados baseline prontos para inclusao nos capitulos de Metodologia e Resultados do TCC.
...
+-- BLUEPRINT_TECNICO.md              # Redirect raiz -> docs/v4.0.0/BLUEPRINT_TECNICO.md
+-- TECHNICAL_BLUEPRINT.md            # Redirect raiz -> docs/v4.0.0/TECHNICAL_BLUEPRINT.md
|
...
|   +-- v3.0.0/
|   |   +-- BLUEPRINT_TECNICO.md      # Blueprint PT v3.0.0 (arquivo historico)
|   |   +-- TECHNICAL_BLUEPRINT.md    # Blueprint EN v3.0.0 (arquivo historico)
|   +-- v4.0.0/
|       +-- BLUEPRINT_TECNICO.md      # Blueprint PT v4.0.0 (versao atual)
|       +-- TECHNICAL_BLUEPRINT.md    # Blueprint EN v4.0.0 (versao atual)
|
...
|   +-- __init__.py
|   +-- start_clinics.sh              # Script de inicializacao rapida (Clinicas A+B)
|   +-- start_all_clinics.sh          # Script de inicializacao completa (todas as 6 clinicas)
|   +-- clinic_a/
|   |   +-- __init__.py
...
+-- tests/                            # Testes e avaliacao
    +-- casos_teste.csv               # Suite de 30 casos de teste (9 categorias)
    +-- executar_testes.py            # Executor de testes em lote (pipeline real)
    +-- avaliar_metricas.py           # Avaliador de metricas (TSR, TCA, HMR)
    +-- logs.jsonl                    # Log estruturado dos resultados de execucao
...
| **Avaliador de Metricas** | Script (`tests/avaliar_metricas.py`) que carrega os resultados de execucao (JSONL) e a especificacao esperada (CSV) para calcular as metricas quantitativas TSR, TCA e HMR. |
...
| **Executor de Testes** | Script (`tests/executar_testes.py`) que automatiza a submissao de casos de teste pelo pipeline completo do sistema (Planner -> Router -> Verifier -> Response Generator), gerando logs estruturados JSONL. |
...
| **HMR (Taxa de Mitigacao de Alucinacoes)** | Metrica que mede a eficacia do Verifier em bloquear respostas inseguras. Calculada como a proporcao de casos com alucinacao detectada que foram bloqueados pelo Verifier. |
...
| **Log Estruturado (JSON Lines)** | Formato de registro onde cada linha do arquivo (`logs.jsonl`) e um objeto JSON independente, contendo campos como `id_caso`, `user_text`, `steps`, `verifier_safe`, `verifier_reason`, `final_response_ok` e `had_raw_hallucination`. Permite processamento programatico dos resultados de teste. |
...
| **Suite de Casos de Teste** | Arquivo CSV (`tests/casos_teste.csv`) contendo 30 casos de teste em 9 categorias de intencao, com especificacao das clinicas e acoes esperadas para cada caso. Serve como ground truth para calculo de metricas. |
...
## Apendice C -- Mapa de Arquivos Reduzido

Versao compacta do mapa de arquivos que generaliza a estrutura repetida das 6 clinicas em um unico padrao. Util para visao geral rapida da organizacao do projeto.
...
|
+-- docs/
|   +-- v{1..4}.0.0/                  # Blueprints versionados (PT + EN por versao)
|       +-- BLUEPRINT_TECNICO.md
|       +-- TECHNICAL_BLUEPRINT.md
|
+-- prompts/                          # System prompts dos modulos LLM
|   +-- planner.txt                   # Decomposicao de tarefas (10 regras)
|   +-- planner_cot.txt               # Variante Chain-of-Thought
|   +-- verifier.txt                  # Validacao de seguranca (3 regras)
|   +-- response_generator.txt        # Geracao de resposta (9 regras)
|
+-- shared/                           # Codigo compartilhado entre clinicas
|   +-- mcp_types.py                  # MCPRequest / MCPResponse (Pydantic)
|   +-- db.py                         # Helpers JSON DB + 4 handlers de agendamento
|
+-- orchestrator_host/                # Cliente MCP -- Orquestrador (4 modulos)
|   +-- main.py                       # Ponto de entrada CLI + Response Generator
|   +-- planner.py                    # Decomposicao de Tarefas
|   +-- router.py                     # Roteamento Dinamico (registro de 6 clinicas)
|   +-- verifier.py                   # Agente Observador
|
+-- clinic_agents/                    # Servidores MCP -- 6 silos federados
|   +-- start_clinics.sh              # Inicializacao rapida (Clinicas A+B)
|   +-- start_all_clinics.sh          # Inicializacao completa (6 clinicas)
|   +-- clinic_{a..f}/                # 6 clinicas (a=Card, b=Derm, c=Card,
|       +-- server.py                 #             d=Orto, e=Orto, f=Derm)
|       +-- db.json                   # Banco de agendamentos (silo isolado)
|
+-- tests/                            # Testes e avaliacao quantitativa
    +-- casos_teste.csv               # 30 casos de teste (9 categorias)
    +-- executar_testes.py            # Executor de testes em lote
    +-- avaliar_metricas.py           # Avaliador de metricas (TSR, TCA, HMR)
    +-- logs.jsonl                    # Log estruturado (resultados de execucao)
    +-- test_cancel_scenario.py       # Teste de integracao: cancelamento
    +-- test_reschedule_scenario.py   # Teste de integracao: reagendamento
    +-- test_three_flows.py           # Teste de integracao: tres fluxos
    +-- test_multi_clinic_nearest.py  # Teste de integracao: multi-clinica
```

**Legenda:**
- `clinic_{a..f}/` representa 6 diretorios identicos em estrutura (`clinic_a/`, `clinic_b/`, ..., `clinic_f/`), cada um com seu proprio `server.py` e `db.json`.
- `v{1..4}.0.0/` representa 4 diretorios de versao (`v1.0.0/`, `v2.0.0/`, `v3.0.0/`, `v4.0.0/`).
- Arquivos `__init__.py` foram omitidos por brevidade.
