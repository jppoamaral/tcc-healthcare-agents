...
|---|---|---|
| 1.0.0 | — | Versao inicial. 4 ferramentas por clinica (`list_patients`, `get_patient`, `check_medications`, `query`). Pipeline de 4 etapas. |
| 2.0.0 | 2025-07 | Catalogo expandido para 7 ferramentas (agendamento, reagendamento, cancelamento). Pipeline de 5 etapas com geracao de resposta pelo Orquestrador. Identificacao do paciente. Historico de conversacao multi-turn. Planner com variante Chain-of-Thought. Verifier com excecoes de PII e serializacao JSON corrigida. Suite de testes de integracao. System prompts extraidos para arquivos externos em `prompts/`. |
...
| **Response Generator** | **Geracao de Resposta** — apos o Verifier aprovar os dados, o Orquestrador transforma os resultados estruturados em uma resposta conversacional em linguagem natural usando Azure OpenAI. O prompt do sistema descreve explicitamente o schema JSON recebido e obriga a apresentacao fiel de todos os dados retornados pelas clinicas. | Capacidade interna do Orquestrador, nao um agente separado. |
...
### 1.4 Pipeline Multi-Agente (5 Etapas)

O pipeline completo do sistema envolve **5 agentes** executando sequencialmente:

```
1. Entrada do usuario  →  [Agente: Planner]      Decomposicao de Tarefas
2. Grafo de passos     →  [Agente: Router]        Despacho Federado
3. Requisicoes MCP     →  [Agente: Clinica A/B]   Execucao Especifica do Dominio
...
### 1.6 Topologia Hub-and-Spoke (Diagrama Mermaid)
...
```json
{
  "reasoning": [
    ...
    "Cardiologia e tratada pela clinic_a.",
    "Preciso listar os horarios disponiveis primeiro.",
...
- Identificadores anonimos como "CARD-001", "DERM-002" — tokens opacos do sistema.
...
```
 Usuario               Planner             Router              Clinica A          Verifier          Response Gen
  |                      |                   |                    |                  |                  |
  |-- consulta --------->|                   |                    |                  |                  |
  |                      |-- grafo passos -->|                    |                  |                  |
  |                      |                   |-- MCPRequest ----->|                  |                  |
...
Ambas as clinicas expoem uma interface de ferramentas identica com **7 ferramentas**, diferenciada apenas pelos dados de dominio subjacentes:
...
|---|---|---|---|
| `list_available_slots` | `doctor: str` *(opcional)* | `{"specialty", "available_slots": [{doctor, specialty, date, time, available}], "note"}` | Retorna todos os horarios disponiveis. Filtro opcional por medico. |
| `book_appointment` | `doctor: str`, `date: str`, `time: str`, `patient_name: str`, `cpf: str` | `{"status": "confirmed", "appointment": {...}, "message"}` | `patient_name` e `cpf` sao injetados automaticamente pelo Orquestrador. |
| `reschedule_appointment` | `original_date: str`, `original_time: str`, `doctor: str`, `new_date: str`, `new_time: str`, `patient_name: str`, `cpf: str` | `{"status": "rescheduled", "original_appointment": {...}, "new_appointment": {...}, "message"}` | Dados originais extraidos do historico de conversacao pelo Planner. |
| `cancel_appointment` | `doctor: str`, `date: str`, `time: str`, `patient_name: str`, `cpf: str` | `{"status": "cancelled", "cancelled_appointment": {...}, "message"}` | Dados extraidos do historico de conversacao pelo Planner. |

### 2.5 Fluxos de Interacao Multi-Turn

O sistema suporta tres fluxos conversacionais principais:

#### Fluxo de Agendamento (2 turnos)
```
Turn 1: "quero marcar uma consulta com cardiologista"
        → Planner gera list_available_slots → Clinica retorna 6 slots
        → Resposta: lista de horarios disponiveis

Turn 2: "pode ser com o Dr. Ricardo dia 21 as 9h"
        → Planner extrai dados do contexto → book_appointment
...
Turn 3: "preciso reagendar para o dia 23 as 8h"
        → Planner extrai consulta original do historico
        → reschedule_appointment com dados originais e novos
...
2. Executar cada consulta atraves do pipeline completo com **structured logging** capturando: saida do Planner, respostas do Router, vereditos do Verifier, resposta final gerada.
...
| 2.2 | Implementar servidor FastAPI da Clinica A (Cardiologia) com banco de dados mock e 7 handlers de ferramentas | `clinic_agents/clinic_a/server.py` |
| 2.3 | Implementar servidor FastAPI da Clinica B (Dermatologia) com banco de dados mock e 7 handlers de ferramentas | `clinic_agents/clinic_b/server.py` |
...
**Entregavel:** Dois Servidores MCP executando independentemente que aceitam requisicoes JSON-RPC 2.0 e retornam dados especificos do dominio, incluindo gestao de agendamentos.
...
| Tarefa | Detalhe | Artefato |
|---|---|---|
| 3.1 | Implementar `Planner` com system prompt restrito incluindo catalogo de 7 ferramentas e suporte a historico de conversacao | `orchestrator_host/planner.py` |
| 3.2 | Implementar variante Chain-of-Thought do Planner (`decompose_cot`) | `orchestrator_host/planner.py` |
| 3.3 | Implementar `Router` com registro de clinicas e despacho JSON-RPC, incluindo injecao automatica de dados do paciente | `orchestrator_host/router.py` |
| 3.4 | Implementar `Verifier` (Agente Observador) com tres regras de seguranca, excecoes de PII e serializacao JSON adequada | `orchestrator_host/verifier.py` |
| 3.5 | Implementar `RESPONSE_SYSTEM_PROMPT` com descricao do schema JSON e regras de apresentacao fiel de dados | `orchestrator_host/main.py` |
| 3.6 | Conectar todos os componentes no loop CLI do `main.py` com identificacao do paciente e historico de conversacao | `orchestrator_host/main.py` |
| 3.7 | Validar pipeline completo: consulta → decomposicao → despacho → verificacao → resposta | Teste CLI ponta a ponta |

**Entregavel:** Um Orquestrador funcional que decompoe consultas, roteia para clinicas, filtra a saida atraves do Verifier e gera respostas em linguagem natural.
...
| Tarefa | Detalhe | Artefato |
|---|---|---|
| 4.1 | Criar teste de cenario de cancelamento (3 turnos: lista → agenda → cancela) | `tests/test_cancel_scenario.py` |
| 4.2 | Criar teste de cenario de reagendamento (3 turnos: lista → agenda → reagenda) | `tests/test_reschedule_scenario.py` |
| 4.3 | Criar teste unificado dos tres fluxos (4 turnos: lista → agenda → reagenda → cancela + validacoes de serializacao e payload) | `tests/test_three_flows.py` |
| 4.4 | Projetar conjunto de testes com >= 30 consultas (normais, casos extremos, adversariais) | `tests/test_queries.json` |
| 4.5 | Adicionar logging estruturado (JSON lines) ao Planner, Router e Verifier | `logs/pipeline_YYYYMMDD.jsonl` |
| 4.6 | Executar conjunto de testes completo e capturar logs | Arquivos de log brutos |
| 4.7 | Calcular TSR, TCA, HR, PVR a partir dos logs | Tabela resumo de metricas |
| 4.8 | Gerar diagramas Mermaid e capturas de tela para apendice da tese | Evidencia visual |
...
|   +-- v2.0.0/
|       +-- BLUEPRINT_TECNICO.md      # Blueprint PT v2.0.0 (versao atual)
|       +-- TECHNICAL_BLUEPRINT.md    # Blueprint EN v2.0.0 (versao atual)
|
+-- prompts/                          # System prompts (arquivos externos)
|   +-- planner.txt                   # Prompt do Planner (decomposicao de tarefas)
|   +-- planner_cot.txt               # Prompt do Planner com Chain-of-Thought
|   +-- verifier.txt                  # Prompt do Verifier (validacao de seguranca)
|   +-- response_generator.txt        # Prompt do Response Generator
|
+-- shared/
|   +-- __init__.py
...
| **Response Generator** | Capacidade interna do Orquestrador que transforma dados estruturados validados em respostas conversacionais em linguagem natural usando LLM. |
